...
File: [`contracts/claimgame/contract.py`](../contracts/claimgame/contract.py) — one contract, ~1,940 lines (v0.3.7, deployed), syntax-verified with `python3 -m py_compile` and `genvm-lint check`. v0.3.8 (see below) is backend/frontend-only — no contract change.
