...
File: [`contracts/claimgame/contract.py`](../contracts/claimgame/contract.py) — one contract, ~1,940 lines (v0.3.7), syntax-verified with `python3 -m py_compile` and `genvm-lint check`.
...
**Deployed API/indexer already** (`fullPageHash` column migration applied, backward-compatible with the still-live v0.3.6 contract via `?? null` guards). **Contract itself and the frontend's new `raiseAppeal` call are NOT yet deployed** — the frontend change is a breaking signature change against the currently-live contract, so it's held back from production until the new contract address is available, consistent with this project's "user deploys the contract, Claude wires in the address after" workflow. Once deployed: re-run `scripts/test-appeal-flow.mjs` (extend it to pass real appeal evidence and confirm `guaranteed_evidence_id` actually gets judged) before treating this round as live-verified.
