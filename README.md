...
| Piece | Where | Notes |
|---|---|---|
| Contract | GenLayer StudioNet | `0x4a4E1a6C3349E88707158fb15bE2F0f6560029Da` — v0.3.7 deployed 2026-08-27, 39/39 methods exercised live with real detailed data (54 checks, 53 passed first try, 1 real consensus disagreement retried and resolved cleanly) — see [genlayer.md](docs/genlayer.md#v037--fourth-re-audit-response-source-verification-full-page-hashing-appeal-specific-evidence-2026-08-27) |
...
- **Final-verdict reliability is empirical, not yet proven at scale**: the deterministic evidence-extraction fix (v0.3.5) and the appeal mechanism (v0.3.6) both work live, but the final semantic judgment still requires leader/validator LLM agreement on verdict and payout band. The v0.3.7 test round (4 real judgment calls) hit one genuine disagreement out of 5 attempts (80% first-try consensus, retry succeeded) — real data, not invented, but still a small sample. A recorded test matrix of 20-50 varied real cases is the honest next step before trusting larger bonds on this.
- **Source-tier verification needs owner curation to actually raise confidence**: v0.3.7's `VERIFIED_PRIMARY` tier requires the contract owner to call `set_protocol_official_domains` per protocol first. Until that happens for a given protocol, its evidence stays `PRIMARY_UNVERIFIED`, which the judgment prompt explicitly treats as grounds to cap confidence at `LOW` — meaning judgments on uncurated protocols route to `NEEDS_HUMAN_REVIEW` more often than `PENDING_APPEAL`, confirmed live in the v0.3.7 test round. This is intentionally conservative, not a bug, but it means the appeal mechanism's newest refinement (appeal-specific evidence) won't be reachable in practice until real protocols get their official domains registered.
...
| **v0.3.7** | **`0x4a4E1a6C3349E88707158fb15bE2F0f6560029Da`** (current) | Owner-curated official-domain verification for source tiers (3-value: VERIFIED_PRIMARY/PRIMARY_UNVERIFIED/CORROBORATIVE), SHA-256 hash of the full normalized page, appeal-specific evidence submission with a guaranteed-judged slot — 39/39 methods live-tested with real detailed data; one genuine consensus disagreement hit and retried successfully (see genlayer.md) |
