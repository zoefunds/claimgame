...
- **Source-tier verification needs owner curation to reach `VERIFIED_PRIMARY`**: `set_protocol_official_domains` is owner-only, and no real protocol has had domains registered yet (the owner's account exists locally but is password-locked — genuinely pending the owner's action, not a code gap). Until then, evidence stays `PRIMARY_UNVERIFIED`. Correction from the v0.3.7 round: this does NOT force every judgment to `NEEDS_HUMAN_REVIEW` — the v0.3.8 reliability matrix showed MEDIUM/HIGH confidence (and `PENDING_APPEAL`) reached on 4 of 6 uncurated-domain cases, since the confidence rule is an influence, not an absolute gate.
